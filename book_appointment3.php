<!-- this is for select option -->
		<?php
			  $db = new mysqli('localhost','root','Andy@2023','login_db');//set your database handler
			  ...
			  $query = "SELECT id, hos_id, hospital_name FROM healthcenter";
			  $result = $db->query($query);

			  while($row = $result->fetch_assoc()){
			    $subcats[$row['hos_id']][] = array("id" => $row['id'], "val" => $row['hospital_name']);
			  }
	...
	<div class="recipient_reg" style="background-color:#272327;">
		...
		<form enctype="multipart/form-data" method="post" class="text-center">
			 <div class="col-md-12">
					...
					<label>
						Hospital: <select id='subcatsSelect' name="subcatsSelect">
